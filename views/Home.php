...
<head>
    ...
    <style>
        .banner { width: 100%; height: 300px; object-fit: cover; margin-bottom: 20px; }
        .category { margin-bottom: 20px; }
        .category-item { text-align: center; padding: 10px; background-color: #f8f9fa; border-radius: 5px; margin: 5px; position: relative; overflow: hidden; }
        .category-item img { width: 100%; height: 150px; object-fit: cover; }
        .category-item a { position: absolute; bottom: 10px; left: 0; right: 0; color: #fff; background-color: rgba(0, 0, 0, 0.6); padding: 5px; text-decoration: none; }
        .category-item a:hover { background-color: rgba(0, 0, 0, 0.8); }
        .product-item { border-bottom: 2px solid #000; padding: 10px 0; }
        .footer { background-color: #343a40; color: white; padding: 20px 0; margin-top: 20px; }
    ...
    .category-item img {
        ...
        border-radius: 8px;
        display: block;
        margin: 0 auto 12px auto; /* tạo khoảng cách 8px dưới ảnh */
    }
    .category-item a {
        display: block;
        margin-top: 12px; /* thêm khoảng cách nhỏ nữa giữa ảnh và chữ */
        font-weight: bold;
        text-decoration: none;
        color: #333;
    }
    .category-item a:hover {
        color: #007bff; /* hover đổi màu cho đẹp */
    }
    </style>
</head>
<body>
        ...
        <?php if (!empty($categories)): ?>
            <?php foreach ($categories as $cat): ?>
                <div class="category-item">
                    ...
                    <a href="?action=category&act=list&category_id=<?= htmlspecialchars($cat['category_id']); ?>">
        ...
        <?php if (!empty($products)): ?>
            ...
            ?>
                <div class="product-item">
                    ...
                    <?php endif; ?>
                    <img src="<?php echo htmlspecialchars($prod['image_product']); ?>" alt="<?php echo htmlspecialchars($prod['name']); ?>">
                    <div class="card-body">
                        ...
                        <p class="card-text">
                            ...
                            <?php endif; ?>
                            <span class="discount-price"><?php echo number_format($discounted_price, 0, ',', '.'); ?>đ</span>
                        </p>
                    </div>
                </div>
            <?php endforeach; ?>
